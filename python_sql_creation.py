...
def to_none_if_nan(x):
    # converts pandas/float NaN to None so MySQL inserts NULL
    if x is None:
        return None
    ...
    with engine.connect() as conn:
        ...
        for _, row in df.head(100).iterrows():
            ...
            rows.append({
                "title": row.get("title"),
                "type": row.get("type"),
                "release_year": to_none_if_nan(row.get("release_year")),
                "date_added": row.get("date_added"),
                "country": row.get("country"),
                "rating": to_none_if_nan(row.get("rating")),
                "description": row.get("description"),
                "language": row.get("language"),
                "popularity": to_none_if_nan(row.get("popularity")),
                "vote_count": to_none_if_nan(row.get("vote_count")),
                "vote_average": to_none_if_nan(row.get("vote_average")),
                "budget": to_none_if_nan(row.get("budget")),
                "revenue": to_none_if_nan(row.get("revenue")),
                "c_id": c_id
            })
